...
const required=['chatContainer','messageInput','sendBtn','sidebar','chatList'];
required.forEach(id=>{
if(!this[id])console.warn('⚠️ Missing element:',id);
});
}
...
// File upload
if(this.attachBtn)this.attachBtn.addEventListener('click',()=>{
...
console.warn('⚠️ Voice recognition not available:',err);
...
updateAINameDisplay(){
if(this.aiNameDisplay)this.aiNameDisplay.textContent=this.settings.aiName;
}
...
scrollToBottom(){
if(!this.chatContainer)return;
setTimeout(()=>{this.chatContainer.scrollTop=this.chatContainer.scrollHeight;},50);
}
